    ...
    do
    {
        ...
        if(ed[h[3]]==a+c-1&&ed[h[2]]==b+d-1)
        {
            ...
            for(int i=0;i<ed[h[0]];i++)
                if(te[0][i]=='.'||te[0][i]==s[h[0]][i])te[0][i]=s[h[0]][i];
                else f=0;

            for(int i=0;i<ed[h[1]];i++)
                if(te[i][0]=='.'||te[i][0]==s[h[1]][i])te[i][0]=s[h[1]][i];
                else f=0;

            for(int i=0;i<ed[h[2]];i++)
                if(te[i][a-1]=='.'||te[i][a-1]==s[h[2]][i])te[i][a-1]=s[h[2]][i];
                else f=0;

            for(int i=0;i<ed[h[3]];i++)
                if(te[b-1][i]=='.'||te[b-1][i]==s[h[3]][i])te[b-1][i]=s[h[3]][i];
                else f=0;

            for(int i=0;i<ed[h[4]];i++)
                if(te[i+b-1][a+c-2]=='.'||te[i+b-1][a+c-2]==s[h[4]][i])te[i+b-1][a+c-2]=s[h[4]][i];
                else f=0;

            for(int i=0;i<ed[h[5]];i++)
                if(te[b+d-2][i+a-1]=='.'||te[b+d-2][i+a-1]==s[h[5]][i])te[b+d-2][i+a-1]=s[h[5]][i];
                else f=0;
            if(f)
                ok();
        }
    }while(next_permutation(h,h+6));
    if(sx==-1)puts("Impossible");
    else
    {
