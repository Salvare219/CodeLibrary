...
inline int cro(long long x,long long y)
{
    ...
    {
        mid=(l+r+1)>>1;
        if(query(1,n-1,2,ed,0,mid)<=y)l=mid;
        else r=mid-1;
    }
    return l;
}
int main()
{
    ...
    sb[n]=sa1[n]=sa2[n]=(1ll<<60);sort(sa2+2,sa2+n);sb[1]=sa2[1]=sa1[1]=0.0;
    sorted[1]=tree[0][1]=0.0;sort(sorted+1,sorted+n);
    build(1,n-1,0);
    k1=k2=1;
    for(int i=2;i<n;i++)
    {
        while(sa1[i]==sa1[i+1])i++,p1[k1]++;
        p1[k1]+=p1[k1-1]+1;temp[k1++]=sa1[i];
    }
    for(int i=0;i<k1;i++)sa1[i]=temp[i];
    for(int i=2;i<n;i++)
    {
        while(sa2[i]==sa2[i+1])i++,p2[k2]++;
        p2[k2]+=p2[k2-1]+1;temp[k2++]=sa2[i];
    }
    for(int i=0;i<k2;i++)sa2[i]=temp[i];
    sa1[k1++]=sa2[k2++]=(1ll<<60);
    int q,ss,bb;scanf("%d",&q);
    long long x,y;
    while(q--)
    {
        scanf("%lld%lld",&x,&y);x*=x;y*=y;
